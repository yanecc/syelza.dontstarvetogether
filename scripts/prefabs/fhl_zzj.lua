...
local function OnGetItemFromPlayer(inst, giver, item)
    if item.prefab == "ancient_soul" and inst.components.finiteuses:GetPercent() < 1 then
        inst.components.finiteuses.current = inst.components.finiteuses.current + TUNING.ZZJ_FINITE_USES
    ...
    for i = 1, numFX do
        inst:DoTaskInTime(math.random() * 0.25, function(inst)
            ...
            if fx then
                ...
                --每根冰柱的伤害
                local dmg = math.random() * 10 * TUNING.ZZJ_PRE
...
local function fn()
    ...
    local function onattack(weapon, attacker, target)
        ...
        if attacker and TUNING.ZZJ_FIREOPEN then
            if TheWorld.state.isnight and math.random() < .04 then
        ...
        if attacker and math.random() < .04 then
