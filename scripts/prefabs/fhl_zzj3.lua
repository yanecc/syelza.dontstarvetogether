...
local function OnGetItemFromPlayer(inst, giver, item)
    if item.prefab == "ancient_soul" and inst.components.finiteuses:GetPercent() < 1 then
        inst.components.finiteuses.current = inst.components.finiteuses.current + TUNING.ZZJ_FINITE_USES * 0.4
    ...
    for i = 1, numFX do
        inst:DoTaskInTime(math.random() * 0.25, function(inst)
            ...
            if fx then
                ...
                --每根冰柱的伤害
                local dmg = math.random() * 40 * TUNING.ZZJ_PRE
...
local function fn()
        ...
        if owner.prefab == "fhl" then
            if owner.level >= 7 then
                ...
                owner:DoTaskInTime(0, function()
                    ...
                    if talker then
                        talker:Say("我需要到7级才能驾驭这把剑!\nI should at least Lv up to lv7!")
                    end
                end)
            end
        else
    ...
    local function onattack(weapon, attacker, target)
        ...
        if attacker and TUNING.ZZJ_FIREOPEN then
            if TheWorld.state.isnight and math.random() < .16 then
        ...
        if attacker and math.random() < .16 then
    ...
    inst.components.weapon:SetDamage(inst.owner.zzjFeedBack + 65 * TUNING.GJBL)
    ...
    inst.components.inventoryitem.atlasname = "images/inventoryimages/fhl_zzj3.xml"
    --inst.components.inventoryitem.keepondeath = true
    inst.components.inventoryitem.imagename = "fhl_zzj3"
...
return Prefab("common/inventory/fhl_zzj3", fn, assets)
